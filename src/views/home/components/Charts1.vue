<template>
  <div id="myChartsBT" :style="{width: '100%', height: '100%'}"></div>
</template>
...
export default {
  ...
  watch: {
    shortTime(newVal, oldVal) {
      // this.dialogVisible = newVal
      this.reportedCount = this.shortTime.reportedCount
      this.noReportedCount = this.shortTime.noReportedCount
      this.otherCount = this.shortTime.otherCount
      this.value = this.shortTime.allCount
      if (this.value !== '') {
        this.drawLine()
      }
    }
  ...
  methods: {
    drawLine() {
      this.$nextTick(_ => {
        const myChart = this.$echarts.init(document.getElementById('myChartsBT'))
        const mockData = [
